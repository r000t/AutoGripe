...
#Stats Reporting
#Set the below value to True to send statistics to the author
#Only the IP address of the attacker will be sent and stored.
sendstats = False
...
def sendemail(ipaddress, recipient, loglines, session):
...
We have detected abuse from the IP address %s. According to the 
WHOIS records for that IP address, this is the proper abuse address for it. 
...
Note: This email was automatically generated by Fail2Ban and sent immediately.
Please use the receive time.''' % (ipaddress, loglines)
...
def processaddress(thisip, logpath, failures=10):
	...
	if len(emailist) != 0:
		if sendstats:
			sent = urllib2.urlopen('https://r000t.com/autogripestats.php?ip=' + thisip)
